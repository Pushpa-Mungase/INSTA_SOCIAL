...
export default function CreatePostModal({ isOpen, setIsOpen, onPostCreated }) {
  ...
  useEffect(() => {
    const fetchPlatforms = async () => {
      try {
        const response = await axiosInstance.get("/platform/get-user-platforms");
        setAvailablePlatforms(response.data.platforms || []);
      } catch (err) {
        console.error("Failed to fetch platforms", err);
      }
  ...
  const handleCreatePost = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.post(apiUrls.createScheduledPost, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
          ...
          {selectedMediaType === "video" && (
            <div className="mt-4">
              ...
              <input
                ...
                multiple
                onChange={(e) => handleFileChange(e, "videoFiles")}
                className="!block w-full text-sm text-gray-700 border border-gray-300 rounded-lg file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100"
              />
            </div>
          )}
